...
> **その後の変化（本Issueの構成は現在のものではない）**: 本Issueが導入した
> 「ホイールオドメトリの pose (x,y,yaw) を EKF で IMU の yaw/vyaw と統合する」
> アーキテクチャは、Issue-21（ヨー源をIMUで補正できない構造欠陥の発見）を経て
> **並進＝ホイールエンコーダ(vx)／回転＝ジャイロ(vyaw) の完全分業**へ置き換わった
> （`wheel_odom` の pose 出力・パターン別 `yaw_scale` 補正は撤去済み）。速度連動の
> 距離補正自体は生き続けているが、値は較正のたびに更新されており本Issueに載せた
> `v_scale_low/high`・`dist_scale_low/high`・`yaw_scale_low/high` は現在の
> `wheel_odom.yaml` には存在しない。現行の値・構成は Issue-10 と CLAUDE.md §6.9
> （N21-2・N21-3）を参照。
...
```mermaid
sequenceDiagram
    participant Mgr as ai_mode_manager_node
    participant Cur as 現在アクティブなノード(例: LLM)
    participant OS as Jetson OS
    participant Next as 次のノード(例: YOLO)

    Mgr->>Cur: deactivate
    Cur->>Mgr: on_deactivate 完了（推論停止）
    Mgr->>Cur: cleanup
    Cur->>Mgr: on_cleanup 完了（プロセス終了・GPUメモリ解放）
    Mgr->>OS: echo 3 > /proc/sys/vm/drop_caches
    OS->>Mgr: ページキャッシュ解放完了
    Mgr->>Next: configure（タイムアウト120s）
    Next->>Mgr: on_configure 完了（モデルロード）
    Mgr->>Next: activate（タイムアウト60s）
    Next->>Mgr: on_activate 完了（推論開始）
```

<p align="center">
  <img src="images/ai_mode_memory_budget.png" alt="LLM/YOLO排他制御のメモリ使用量概算" width="80%">
</p>
...
<p align="center">
  <img src="images/llm_crosslingual_latency.png" alt="日本語プロンプトと英語プロンプトのLLM判定時間比較" width="60%">
</p>

```mermaid
flowchart LR
    A["日本語音声認識テキスト\n(例: 物体検索開始)"] --> B{プロンプト言語}
    B -->|日本語プロンプト| C["セマンティック干渉\n(『開始』→ start_mapping)"]
    C --> D["❌ 誤分類\n(~15s)"]
    B -->|英語プロンプト\n(Crosslingual)| E["DISAMBIGUATION RULES\nで明示的に分離"]
    E --> F["✅ search_object\n(~0.8s)"]
```
...
# Issue-10: レーザーオドメトリを較正した結果、あえてEKFへ統合しなかった判断

## 事象

車輪スリップ時、並進速度（vx）の唯一の情報源であるホイールエンコーダが汚染される
問題（Issue-02 で rf2o から乗り換えた側の弱点）に対し、LiDAR スキャンから独立に
並進を推定する `laser_odom_node` を新設した。回転はジャイロから既知として外部注入し、
探索を並進2自由度だけに縮退させる設計（一般的な ICP 系レーザーオドメトリが回転・並進を
同時推定して不安定化するアパーチャ問題を、そもそも避ける構成）。

これを EKF (`robot_localization`) の追加観測源として統合してよいかを、実機3セッション・
約100万行のオドメトリログをオフライン較正して判断した。

## 影響

判断を誤ると2方向のリスクがある: 共分散を過小申告したまま統合すれば「嘘の確信」を
フィルタに渡し発散を招く。逆に、検証不足のまま見送れば、スリップ耐性を上げる機会を
逃す。
...
| # | 仮説 | 検証方法 | 結果 |
|---|------|----------|------|
| A | 申告共分散は信頼でき、そのまま EKF の `odom1` として統合してよい | `z = (v_laser − v_wheel) / σ` の分布をオフライン較正（GUARD発火区間は除外） | 棄却: `std(z) = 0.33`（理想 1.0）で約3倍の**過大申告**。安全側だが平時の重み占有率は中央値18〜22%に留まり、統合してもほとんど動かない |
| B | 縮退判定（アパーチャ問題対策）が実機でも機能している | gate が開いた24,572サイクルでの縮退発火率を集計 | 棄却: 発火 **0件**。閾値が実測分散の約40倍の余裕を持ち、この部屋の幾何では原理的に到達しない設計になっていた |
| C | スリップ時の重み比向上分だけでも EKF 統合の価値がある | スリップ中（wheel側cov ×20）・棄却中（×50）の laser 重み占有率を試算 | 棄却: 該当区間の重みは81〜93%まで跳ね上がり実質的な主測定に転じるが、この便益は「独立した第3の証言者としてスリップを検知し、車輪側を0へ落とす」という別経路（後述）で既に回収済み。EKFにも統合すると証言者と主測定を兼ねる単一障害点になる |

### 検証データ

```
z = (v_laser − v_wheel) / σ の較正（3セッション合算 n=22,087）
  std(z)      = 0.33   (理想 1.0 → 約3倍の過大申告)
  |z|<=1      = 98.8%  (理想 68%)
  相関係数     = 0.68 〜 0.84（セッション別）

縮退判定（UNKNOWN_COV への退避）の発火率
  gate開 24,572 サイクル中 0 件

EKF統合時の想定重み占有率（中央値、wheel側 cov=0.001 に対する相対値）
  平常時          17.7 〜 21.7 %
  スリップ中(×20) 81.1 〜 84.7 %
  棄却中(×50)     91.5 〜 93.3 %
```
...
<p align="center">
  <img src="images/ekf_z_distribution.png" alt="laser_odom申告共分散の較正: z分布" width="75%">
</p>

<sub>注記: 実機の生z値（n=22,087）はリポジトリに保存されていないため、上図は記録済みの
要約統計（std=0.33）から正規分布を仮定して再構成した代表的な分布であり、実測データ
そのものではない。</sub>

<p align="center">
  <img src="images/ekf_weight_occupancy.png" alt="EKF統合を想定した場合のlaser_odom重み占有率" width="75%">
</p>

## 対策

EKF への統合は見送り、`laser_odom_node` の出力は**独立したスリップ検知の第3軸**として
使う構成に確定した。既存の判定（回転軸・直進軸、いずれもIMU参照）は衝突などの慣性
ショックで車輪とIMUが同時に「動いた」と誤認しうる弱点を持つが、環境基準で並進を測る
laser はこの種の慣性ショックに引きずられない独立した証拠になる。

```python
# 「車輪は動いていると言うが、環境に対しては動いていない」を検出するだけの二値判定
# 速度の値そのものはEKFにもwheel_odomの出力にも入らない
def evaluate_translation_slip_axis(v_wheel, v_laser, th):
    if v_laser is None:      # 証拠なし＝フェイルオープン
        return False
    return abs(v_wheel) > th.v_wheel_high and abs(v_laser) < th.v_laser_low
```

発火すると車輪側の `vx` を実際に `0` へ強制し、共分散を50倍にブーストする
（＝「測定できていない」に近い扱い）。laser の値そのものは publish されず、
二値判定の結果だけが下流に伝わる。

## 結果

実機での初回発火（衝突直後の慣性ショック局面）で、既存の直進軸（IMU参照）は
IMU側も加速度スパイクを示したため発火せず、laser 基準の第3軸だけがスリップを
正しく捕捉した。EKF は無変更のまま、車輪スリップへの耐性だけが1本増えた。

## 学び

「較正して精度不足と分かったら統合しない」という単純な話ではなかった —
共分散は**過大**申告（安全側）だったにもかかわらず統合を見送ったのは、
便益（スリップ時の重み向上）が既に**別経路で回収済み**であり、EKFにも入れると
1つのセンサが「棄却の根拠」と「棄却後の主測定」を兼ねて単一障害点になるという、
精度とは別軸のシステム設計上の理由による。較正データが「使えそう」でも、
それをどこに配線するかは別の設計判断が要る。
